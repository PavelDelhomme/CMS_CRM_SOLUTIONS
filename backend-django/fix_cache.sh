...
docker exec vtcbuilder-backend find /app -name "*.pyc" -delete 2>/dev/null || echo "⚠️  Conteneur non accessible"
docker exec vtcbuilder-backend find /app -name "__pycache__" -type d -exec rm -rf {} + 2>/dev/null || echo "⚠️  Conteneur non accessible"
...
docker exec vtcbuilder-backend grep -n "Sum('total" /app/api/views.py | head -2
