...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        demo_tenant = Tenant.objects.filter(slug='demo-vtc-company').first()
        ma_societe = Tenant.objects.filter(slug='ma-societe-vtc').first()
        ...
        # Résumé
        remaining_tenants = Tenant.objects.filter(deleted_at__isnull=True).exclude(slug='public').count()
        self.stdout.write('')
