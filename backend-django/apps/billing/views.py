...
class SubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def tenants_without_subscription(self, request):
        ...
        try:
            ...
            tenants_without = Client.objects.filter(
        ...
        try:
            ...
            if user.is_super_admin():
                ...
                try:
                    ...
                    error_response = Response(
                        {'error': 'Validation error', 'details': str(e) if django_settings.DEBUG else 'Invalid data'},
                        status=status.HTTP_400_BAD_REQUEST
                    )
                    add_cors_headers(error_response, request)
                    return error_response
                ...
                if tenant_from_validated:
                    if isinstance(tenant_from_validated, Client):
                        ...
                        tenant = Client.objects.get(id=tenant_from_validated.id)
                    else:
                        tenant = None
                else:
                    tenant = None
