        ...
        try:
            ...
            stats = {
                'overview': {
                    ...
                    'suspended_tenants': Client.objects.filter(status='suspended', deleted_at__isnull=True).count(),
                    'trial_tenants': Client.objects.filter(status='trial', deleted_at__isnull=True).count(),
                    'cancelled_tenants': Client.objects.filter(status='cancelled', deleted_at__isnull=True).count(),
                ...
                'activity': {
                    'tenants_today': Client.objects.filter(
                    ...
                    'tenants_this_week': Client.objects.filter(
                    ...
                    'tenants_this_month': Client.objects.filter(
            ...
            try:
                ...
                templates_usage = []
                reference_tenant = Client.objects.filter(deleted_at__isnull=True).first()
                if reference_tenant:
            ...
            try:
                tenants_by_status = Client.objects.filter(deleted_at__isnull=True).values('status').annotate(count=Count('id')).order_by('-count')
            ...
            # Add recent tenants
            try:
                recent_tenants = Client.objects.filter(deleted_at__isnull=True).order_by('-created_at')[:10]
