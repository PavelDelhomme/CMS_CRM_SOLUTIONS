...
export default function EditServicePage() {
  ...
  const loadService = async () => {
    try {
      setLoading(true)
      const service = await serviceService.getById(parseInt(serviceId))
      reset({
        name: service.name,
  ...
  const onSubmit = async (data: ServiceForm) => {
    setSaving(true)
    try {
      await serviceService.update(parseInt(serviceId), data)
  ...
  return (
    <TenantLayout 
      title="Édition Service" 
      subtitle="Modifier un service de transport"
    >
