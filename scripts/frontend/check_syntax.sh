...
if ! docker ps | grep -q cms_crm_frontend; then
...
TSC_OUTPUT=$(docker exec cms_crm_frontend npm run type-check 2>&1 || true)
...
ESLINT_OUTPUT=$(docker exec cms_crm_frontend npm run lint 2>&1 || true)
